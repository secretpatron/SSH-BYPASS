...
if ! tmux has-session -t ssh-session 2>/dev/null; then
  ...
  while true; do
    sshpass -p "segfault" ssh -L5900:localhost:5900 -o 'SetEnv SECRET=IKchmmqmqWXCmHKqmTEDilqd' -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@lulz.segfault.net "bash ok.sh"
